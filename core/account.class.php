...
class Account
{
    ...
    public function getCharacterList() {
        if ($this->characters === null) {
            ...
            $sth = UltimaPHP::$db->prepare($query);
            $sth->execute(array(":account_uid" => $this->uid));
            $result = $sth->fetchAll(PDO::FETCH_ASSOC);
            
            $chars = array();
            foreach ($result as $char) {
                $chars[] = array('uid' => $char['id'], 'name' => $char['name']);
            }
            return $chars;
        } 
        else {
    ...
    public function loginCharacter($info = array()) {
        if (isset($this->characters[$info['slotchoosen']])) {
            ...
            Sockets::addEvent($this->client, array("option" => "player", "method" => "playMusic", "args" => 29), 0.0);
            Sockets::addEvent($this->client, array("option" => "player", "method" => "setWeather", "args" => 2), 0.0);
            Sockets::addEvent($this->client, array("option" => "player", "method" => "setSeasonal", "args" => 1), 0.0);
            Sockets::addEvent($this->client, array("option" => "player", "method" => "setLight", "args" => 0), 0.0);
            Sockets::addEvent($this->client, array("option" => "player", "method" => "updateCursorColor", "args" => 0), 0.0);
            Sockets::addEvent($this->client, array("option" => "player", "method" => "drawChar"), 0.0);
            Sockets::addEvent($this->client, array("option" => "player", "method" => "updateStatusBar"), 0.0);
            Sockets::addEvent($this->client, array("option" => "player", "method" => "drawPlayer"), 0.0);
            Sockets::addEvent($this->client, array("option" => "player", "method" => "mountSpeed"), 0.0);
            Sockets::addEvent($this->client, array("option" => "player", "method" => "statusBarInfo"), 0.0);
            Sockets::addEvent($this->client, array("option" => "player", "method" => "extendedStats"), 0.0);
            Sockets::addEvent($this->client, array("option" => "player", "method" => "drawPlayer"), 0.0);
