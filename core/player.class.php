...
class Player
{
    ...
    function __construct($client = null, $character_uid = null) {
        if ($client === null || $character_uid === null) {
            return false;
        }
        ...
        $sth->execute(array(":player_uid" => $this->uid));
        ...
        if (isset($result[0])) {
            ...
            $this->position = array('x' => $position[0], 'y' => $position[1], 'z' => $position[2], 'map' => $position[3], 'facing' => 6);
    ...
    public function sendClientLocaleBody() {
        $body_type = $this->body;
        $pos = array('x' => $this->position['x'], 'y' => $this->position['y'], 'z' => $this->position['z'], 'facing' => $this->position['facing']);
    ...
    public function updateCursorColor($color = 0) {
        $packet = "BF0006000800";
         // . str_pad(dechex($color), 2, "0", STR_PAD_LEFT);
        Sockets::out($this->client, $packet);
    }
    ...
    public function setWeather($weather = null, $effect = 0, $temperature = 10) {
        ...
        echo "Uncompressed packet: $packet\n";
        Sockets::out($this->client, $packet);
    }
